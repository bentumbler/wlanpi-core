...
class ConnectionManager:
    ...
    def _cancel_deadline(self, client: dict[str, Any]) -> None:
        ...
        Never cancels the current task: when the timer itself is the one
        stopping the capture, cancelling it would abort that teardown.
        """
    ...
    async def _expire_after(self, websocket: WebSocket, duration_sec: int) -> None:
        """Stop a bounded capture once its duration elapses."""
    ...
    async def _end_session(
        ...
        await self._notify_subscribers(
            ...
            session_id,
            code,
            {"message": message, "session_id": session_id, "reason": reason},
            final=True,
        )
