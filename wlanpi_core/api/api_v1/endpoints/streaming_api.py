...
async def websocket_endpoint(websocket: WebSocket) -> None:
    ...
    **Sessions & subscribers:** `start` returns a `session_id` in the
    ...
    `session_id` from any socket of the same `did`. Session descriptors carry
    `config`, lifetime fields, `owner_attached`, and `subscriber_count`.
        ...
        while True:
            ...
            if command == "get_supported_frequencies":
                ...
                try:
                    ...
                    await manager.apply_configuration(websocket, validated_configs.root)
