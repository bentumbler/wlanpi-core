...
Companion to the MCP rollout plan and the capture WebSocket handover doc
(`docs/mcp-rollout.md`, `docs/capture-ws-mcp-handover.md` on the capture
branches until #165 merges). This is a **separate concern from the capture
WebSocket** and ships as its own PR series (see §7). Nothing here changes the
capture protocol.
...
- **S1 implemented** (`feature/supplicant-log-store`): store + sidecar
  (`wlanpi_core/wpa/supplicant_log.py`), `debug_level` on `RootConfig`,
  `-d`/`-dd` on the launcher, `?debug_level=` override and `connections[]` on
  activate, monitor writes outcome and MLD status keys, deactivate closes the
  sidecar, dead `parse_wpa_log` removed, four matrix rows. Not in S1: the
  per-file size cap from §2.3 (the monitor only lives 15 s; a size check
  belongs with the S2 read path, which can report `size_bytes` and refuse to
  return more than a tail beyond a threshold).
- S2 (read API + event parser), S3, S4: not started.
